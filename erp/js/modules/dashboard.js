...
<div class="stats-grid">
  ...
  <div class="stat-card">
    ...
    <div>
      ...
      <div class="stat-delta ${overdueInvoices > 0 ? 'down' : 'up'}">
        ${overdueInvoices > 0 ? '<i class="fas fa-exclamation-circle"></i> ' + overdueInvoices + ' fact. vencidas' : '<i class="fas fa-check-circle"></i> Al dia'}
      </div>
    </div>
  </div>
  <div class="stat-card">
  ...
  <div class="stat-card">
    ...
    <div>
      ...
      <div class="stat-label">Ordenes de Compra</div>
  ...
  <div class="stat-card">
    ...
    <div>
      ...
      <div class="stat-label">Tareas en Ejecucion</div>
...
<div class="grid-3 mb-2">
  ...
  <div class="card">
    <div class="card-header">
      <span class="card-title"><i class="fas fa-chart-line text-primary"></i> Facturacion vs Cobros</span>
    </div>
    <div class="card-body">
        ...
        <table>
          ...
          <tbody>
            ${projects.map(p => {
              ...
              return '<tr><td><strong>' + p.name + '</strong><br><span class="text-muted" style="font-size:11px">' + (p.client||'') + '</span></td><td style="min-width:120px"><div class="progress-bar"><div class="progress-fill" style="width:' + avgProgress + '%"></div></div><span style="font-size:11px;color:var(--text-muted)">' + avgProgress + '%</span></td><td>' + statusBadge(p.status) + '</td><td class="number-cell">' + fmtMoney(p.budget) + '</td></tr>';
            }).join('')}
          </tbody>
        </table>
      </div>
...
<div class="card mt-3">
  ...
  <div class="card-body" style="padding:0">
    <div class="table-wrap">
      <table>
        <thead><tr><th>Numero</th><th>Proyecto</th><th>Cliente</th><th>Total</th><th>Estado</th><th>Vencimiento</th></tr></thead>
        <tbody>
          ${invoices.slice(0,5).map(inv => {
            const proj = DB.getById('projects', inv.project_id);
            const rowClass = (isOverdue(inv.due_date) && inv.status !== 'paid') ? 'text-danger fw-bold' : '';
            return '<tr><td class="nowrap"><strong>' + inv.number + '</strong></td><td>' + (proj ? proj.name : '-') + '</td><td>' + (inv.client_name||'') + '</td><td class="number-cell">' + fmtMoney(inv.total) + '</td><td>' + statusBadge(inv.status) + '</td><td class="' + rowClass + '">' + fmtDate(inv.due_date) + '</td></tr>';
          }).join('')}
        </tbody>
      </table>
    </div>
...
function buildAlerts(invoices, pos, tasks) {
  ...
  pos.filter(p => p.status === 'sent').forEach(po => {
    ...
    alerts.push({ type: 'warning', icon: 'fa-clock', msg: 'OC ' + po.number + ' pendiente de recepcion — ' + fmtMoney(po.total) });
  });
...
function renderProjectsChart(projects) {
  ...
  new Chart(ctx, {
    type: 'doughnut',
    data: {
      labels: ['Activos', 'Planificacion', 'Completados', 'Pausados'],
