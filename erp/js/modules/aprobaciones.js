...
function _apprBuildLicConfig() {
  ...
  html += '<div style="font-size:12px;color:var(--text-muted);margin-bottom:14px">Definí quiénes pueden aprobar cada paso. Cualquier usuario marcado podrá aprobar ese nivel. Se auto-asignan al adjudicar.</div>';

  if (!usuarios.length) {
    html += '<div style="text-align:center;padding:20px;color:var(--text-muted);font-size:13px">No hay usuarios cargados en el sistema.</div>';
  } else {
    steps.forEach(function(step) {
      var sc = stepsCfg[step.key] || {};
      var curIds = sc.user_ids || [];

      html += '<div style="margin-bottom:10px;padding:12px;border:1px solid var(--border);border-radius:8px;background:var(--bg)">';
      html += '<div style="font-weight:600;font-size:13px;margin-bottom:8px">' + step.label + '</div>';
      ...
      usuarios.forEach(function(u) {
        ...
        html += '<label style="display:flex;align-items:center;gap:8px;font-size:13px;cursor:pointer">' +
          '<input type="checkbox" id="lic-cfg-' + step.key + '-' + u.id + '" value="' + u.id + '"' + checked + '> ' +
          escHtml((u.name || u.email || u.id) + (u.role ? ' — ' + u.role : '')) +
        '</label>';
      });
      html += '</div></div>';
    });
...
function apprSaveLicConfig() {
  ...
  steps.forEach(function(key) {
    var ids = [];
    usuarios.forEach(function(u) {
      var cb = document.getElementById('lic-cfg-' + key + '-' + u.id);
      if (cb && cb.checked) ids.push(u.id);
    });
    stepsCfg[key] = { user_ids: ids };
  });
